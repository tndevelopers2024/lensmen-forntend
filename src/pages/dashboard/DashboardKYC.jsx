...
const DashboardKYC = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!KYC_DOCS.every(d => files[d.key])) { toast.error('Please select all 4 documents'); return }
  ...
  const twoCol = !isMobile && status !== 'Approved' && hasSubmitted
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      <div>
        ...
        <h1 style={{ fontSize: isMobile ? 20 : 28, fontWeight: 700, color: INK, margin: 0, letterSpacing: '-0.02em' }}>KYC Documents</h1>
      </div>
      ...
      {/* Upload + Submitted */}
      <div style={{ display: 'grid', gridTemplateColumns: twoCol ? '1fr 1fr' : '1fr', gap: 16, alignItems: 'start' }}>

        {/* Upload Form */}
        {status !== 'Approved' && (
          <div style={{ background: '#fff', borderRadius: 14, border: `1px solid ${LINE}`, padding: isMobile ? 16 : 24 }}>
            ...
            <div style={{ fontSize: 12, color: '#9ca3af', marginBottom: 18 }}>
              Submit all 4 documents to request verification.
            </div>
                  ...
                  return (
                    <div key={doc.key}>
                      ...
                      <label style={{ display: 'block', cursor: 'pointer', position: 'relative' }}>
                        <div style={{
                          border: `2px dashed ${file ? NAVY : '#e5e7eb'}`,
                          borderRadius: 12, padding: isMobile ? '16px 8px' : '20px 12px',
                          textAlign: 'center', background: file ? '#eef2ff' : '#fafafa',
                          transition: 'all 0.15s',
                        }}>
                          {file ? (
                            <>
                              <HiCheckCircle style={{ fontSize: 22, color: '#10b981', margin: '0 auto 6px' }} />
                              <div style={{ fontSize: 10, fontWeight: 600, color: NAVY, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', padding: '0 4px' }}>
                                {file.name}
                              </div>
                            </>
                          ) : (
        ...
        {hasSubmitted && (
          <div style={{ background: '#fff', borderRadius: 14, border: `1px solid ${LINE}`, padding: isMobile ? 16 : 24 }}>
            <div style={{ fontWeight: 600, color: INK, fontSize: 14, marginBottom: 3 }}>Submitted Documents</div>
            <div style={{ fontSize: 12, color: MUTED, marginBottom: 16 }}>Documents currently on file.</div>
            <div style={{
              display: 'grid',
              gridTemplateColumns: twoCol ? '1fr 1fr' : 'repeat(2, 1fr)',
              gap: 12,
            }}>
              {KYC_DOCS.map(doc => {
                ...
                return (
                  <div key={doc.key} style={{ background: '#f9fafb', borderRadius: 12, padding: '10px', border: `1px solid ${LINE}` }}>
                    ...
                    <div style={{ position: 'relative', borderRadius: 8, overflow: 'hidden', height: 90, background: '#fff', border: '1px solid #e5e7eb' }}>
                      ...
                      >
                        <HiEye style={{ color: '#fff', fontSize: 20, opacity: 0, transition: 'opacity 0.2s' }}
