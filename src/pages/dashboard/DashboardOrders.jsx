...
import {
  ...
  HiLocationMarker, HiChevronRight, HiOutlineShoppingBag,
} from 'react-icons/hi'

const BRAND = '#1e1b4b'   // accent — indigo (no orange)
const NAVY  = '#1e1b4b'
const SOFT  = '#eef2ff'   // light indigo tint for fills
  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        ...
        <h1 style={{ fontSize: 24, fontWeight: 700, color: NAVY, margin: 0, letterSpacing: '-0.02em' }}>My Orders</h1>
      </div>
      ...
      {/* Orders list */}
      {filtered.length === 0 ? (
        <div style={{ background: '#fff', borderRadius: 16, padding: '64px 24px', textAlign: 'center', border: '1px solid #eef0f3' }}>
          <div style={{ width: 56, height: 56, borderRadius: 14, background: '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
            <HiOutlineShoppingBag style={{ fontSize: 26, color: '#94a3b8' }} />
          </div>
          <div style={{ fontSize: 15, fontWeight: 600, color: '#334155', marginBottom: 4 }}>No orders yet</div>
          <div style={{ fontSize: 13, color: '#94a3b8', marginBottom: 20 }}>Your rentals in this category will appear here.</div>
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                    <div style={{ fontWeight: 700, color: NAVY, fontSize: 14, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  ...
                  <div style={{ fontSize: 12, color: '#9ca3af', display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <span>{order.totalDays || 1} day{order.totalDays !== 1 ? 's' : ''}</span>
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontWeight: 700, color: NAVY, fontSize: 16 }}>₹{order.totalPrice?.toLocaleString()}</div>
                  </div>
                  <HiChevronRight style={{ color: '#cbd5e1', fontSize: 18 }} />
                </div>
              </div>
            )
          })}
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ padding: '28px 28px 0' }}>
              <div style={{ marginBottom: 22 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <h2 style={{ fontSize: 20, fontWeight: 700, color: NAVY, margin: 0, letterSpacing: '-0.02em' }}>Order Details</h2>
                  <StatusBadge status={selected.status} />
                </div>
                <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 6, fontFamily: 'monospace' }}>Ref: #{selected._id?.slice(-8).toUpperCase()}</div>
              </div>

              {/* Stepper */}
              <div style={{ background: '#f8fafc', border: '1px solid #eef0f3', borderRadius: 14, padding: '18px 12px', marginBottom: 24 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  <div>
                    ...
                    <div style={{ background: '#f8fafc', borderRadius: 12, padding: '14px', border: '1px solid #eef0f3' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                        <div>
                          <div style={{ fontSize: 9, color: '#94a3b8', fontWeight: 600, textTransform: 'uppercase', marginBottom: 3, letterSpacing: '0.06em' }}>Start</div>
                          <div style={{ fontSize: 13, fontWeight: 600, color: NAVY }}>{new Date(selected.startDate).toLocaleDateString('en-GB')}</div>
                          <div style={{ fontSize: 11, color: '#64748b', marginTop: 1 }}>{new Date(selected.startDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                        </div>
                        <HiArrowRight style={{ color: '#cbd5e1', alignSelf: 'center' }} />
                        <div style={{ textAlign: 'right' }}>
                          <div style={{ fontSize: 9, color: '#94a3b8', fontWeight: 600, textTransform: 'uppercase', marginBottom: 3, letterSpacing: '0.06em' }}>Return</div>
                          <div style={{ fontSize: 13, fontWeight: 600, color: NAVY }}>{new Date(selected.endDate).toLocaleDateString('en-GB')}</div>
                          <div style={{ fontSize: 11, color: '#64748b', marginTop: 1 }}>{new Date(selected.endDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                        </div>
                      </div>
                      <div style={{ borderTop: '1px solid #e5e7eb', paddingTop: 10, display: 'flex', justifyContent: 'space-between', fontSize: 12 }}>
                  ...
                  <div>
                    ...
                    <div style={{ background: '#f8fafc', border: '1px solid #eef0f3', borderRadius: 12, padding: '12px 14px', borderLeft: `3px solid ${NAVY}` }}>
                      <div style={{ fontWeight: 600, color: NAVY, fontSize: 13, marginBottom: 4 }}>Lensmen Rentals HQ</div>
                      <div style={{ fontSize: 11, color: '#64748b', lineHeight: 1.6 }}>
                        123 Creative Studio Street,<br />Film City, Chennai - 600001
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {/* Total + Cancel */}
              <div style={{ background: NAVY, borderRadius: 14, padding: '18px 22px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                <div>
                  ...
                  <div style={{ fontSize: 24, fontWeight: 700, color: '#fff' }}>₹{selected.totalPrice?.toLocaleString()}</div>
                </div>
                <StatusBadge status={selected.status} />
              </div>
            </div>
            ...
            <div style={{ padding: '0 28px 24px' }}>
              {!['Returned', 'Closed', 'Rejected'].includes(selected.status) && (
                <button
                  onClick={() => setConfirming(selected._id)}
                  style={{
      ...
      {confirming && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: 16 }}>
          <div style={{ background: '#fff', borderRadius: 18, padding: 30, maxWidth: 360, width: '100%', textAlign: 'center', boxShadow: '0 24px 64px rgba(0,0,0,0.28)' }}>
            <div style={{ width: 52, height: 52, borderRadius: '50%', background: '#fef2f2', color: '#ef4444', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px', fontSize: 22 }}>
              <HiExclamationCircle />
            </div>
            <h3 style={{ fontSize: 18, fontWeight: 700, color: NAVY, margin: '0 0 8px' }}>Cancel this order?</h3>
            <p style={{ fontSize: 13, color: '#64748b', fontWeight: 400, marginBottom: 24, lineHeight: 1.6 }}>
              This action cannot be undone. Your items will be released back to inventory.
            </p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <button onClick={() => setConfirming(null)} style={{ padding: '11px', background: '#f1f5f9', border: 'none', borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: 'pointer', color: '#475569' }}>
                Keep it
              </button>
              <button
                onClick={() => { cancelOrder(confirming); setConfirming(null); setSelected(null) }}
