...
const ProductDetails = ({ setShowBookingModal }) => {
  ...
  if (!product) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-6">
        ...
        <h2 className="text-2xl font-black text-[#03045e] uppercase tracking-widest">Gear Not Found</h2>
        <button onClick={() => navigate('/')} className="text-[#5e60ce] font-black uppercase tracking-widest text-xs hover:underline">Return to Inventory</button>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 lg:p-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
      ...
      <button 
        onClick={() => navigate('/')} 
        className="group mb-12 flex items-center space-x-3 text-slate-400 hover:text-[#5e60ce] transition-colors"
      >
        <div className="w-8 h-8 rounded-full bg-white border border-slate-100 flex items-center justify-center group-hover:bg-[#5e60ce] group-hover:border-[#5e60ce] transition-all shadow-sm">
          <HiArrowLeft className="text-sm group-hover:text-white" />
        </div>
        <span className="font-black uppercase tracking-[0.2em] text-[10px]">Back to Catalog</span>
      </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-white p-2 rounded-[1.5rem] shadow-xl shadow-slate-200/50 border border-slate-50 relative overflow-hidden group">
            ...
            <div className="absolute top-4 left-4">
              <div className={`flex items-center space-x-2 px-3 py-1 rounded-full backdrop-blur-md shadow-lg border ${product.isAvailable ? 'bg-green-500/10 border-green-500/20 text-green-600' : 'bg-red-500/10 border-red-500/20 text-red-600'}`}>
                ...
                <span className="font-black uppercase tracking-widest text-[8px]">{product.isAvailable ? 'Available Now' : 'Currently Rented'}</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-slate-50/50 p-4 rounded-2xl border border-slate-100 text-center">
              <HiShieldCheck className="text-lg text-[#5e60ce] mx-auto mb-2" />
              <p className="font-black text-[8px] uppercase tracking-widest text-slate-400">Insured</p>
            </div>
            <div className="bg-slate-50/50 p-4 rounded-2xl border border-slate-100 text-center">
              <HiClock className="text-lg text-[#00b4d8] mx-auto mb-2" />
              <p className="font-black text-[8px] uppercase tracking-widest text-slate-400">24/7 Support</p>
            </div>
            <div className="bg-slate-50/50 p-4 rounded-2xl border border-slate-100 text-center">
              <HiCheckCircle className="text-lg text-green-500 mx-auto mb-2" />
              <p className="font-black text-[8px] uppercase tracking-widest text-slate-400">Verified</p>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-7 space-y-8">
          <div className="space-y-4">
            <div className="inline-block px-3 py-1 bg-indigo-50 border border-indigo-100 rounded-md">
              <p className="text-[#5e60ce] font-black uppercase tracking-[0.2em] text-[8px]">Premium Equipment</p>
            </div>
            <h1 className="text-[16px] font-black text-[#03045e] leading-tight uppercase tracking-tight">{product.name}</h1>
            ...
            <div className="flex items-center space-x-4 pt-4 border-t border-slate-100">
              ...
              <p className="text-slate-400 font-bold uppercase tracking-widest text-[9px]">Rental<br/>Rate / Day</p>
            </div>
          </div>
          ...
          <div className="pt-6 space-y-3">
            <h3 className="font-black text-[#03045e] uppercase tracking-widest text-[14px]">Technical Specifications</h3>
            <div className="space-y-2">
              {[
                ...
                <div key={i} className="flex justify-between items-center py-2 border-b border-slate-50">
                  <span className="text-slate-400 text-[9px] uppercase font-bold tracking-widest">{spec.label}</span>
                  <span className="text-[#03045e] text-[9px] uppercase font-black tracking-tight">{spec.value}</span>
                </div>
              ))}
            </div>
          </div>
